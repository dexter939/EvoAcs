...
# Recent Changes
- **October 21, 2025 - Auto-Update System**: ✅ **SISTEMA DI AUTO-UPDATE SOFTWARE COMPLETO** - Sistema carrier-grade per gestione automatica deployment e migrations con: (1) Database: system_versions table (12 colonne, 5 indexes) con enum constraints per deployment_status/environment. (2) Model: SystemVersion environment-aware con getCurrentVersion($env), transactional recordDeployment/markAsFailed, automatic rollback su failure. (3) Service: SystemUpdateService con auto-migrations runner, 5 health checks (database/cache/storage/queue/critical_tables), duplicate deployment prevention. (4) CLI: RunSystemUpdate command con progress bar interattiva. (5) API: 6 REST endpoints environment-aware (/status, /version, /health, /history, /update). (6) Integration: build.sh auto-esegue system:update ad ogni deployment. **Architect-reviewed, production-ready**.
