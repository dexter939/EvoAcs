...
# Recent Changes
- **October 21, 2025 - GitHub Auto-Update System**: ✅ **SISTEMA GITHUB AUTO-UPDATE CARRIER-GRADE COMPLETO** - Sostituito sistema Replit-triggered con GitHub Releases-based workflow completo: (1) Database: 10 nuovi campi GitHub nella system_versions table (github_release_url, github_release_tag, download_path, package_checksum SHA256, approval_status enum pending/approved/rejected, approved_by, approved_at, scheduled_at, changelog, release_notes) + 2 indexes (approval_status, github_release_tag). Migration cross-database compatible (fix PostgreSQL-specific queries). (2) Model: SystemVersion esteso con approval workflow (approve/reject/scheduleDeployment methods + pendingApproval/approved scopes). (3) Services: GitHubReleaseService (Replit GitHub connector integration con auto-token, getLatestRelease, downloadRelease, validateChecksum SHA256, extractPackage .tar.gz/.zip, compareVersions semantic), UpdateStagingService (stageUpdate download+extract, validateStagedUpdate 4-checks package/checksum/files/disk, cleanupStagedUpdate), UpdateApplicationService (applyUpdate transactional con createBackup automatic, copyFiles exclude vendor/node_modules/storage/.env/.git, runMigrations parsing, rollback automatic on failure, clearCaches, health checks integration). (4) CLI: CheckGitHubUpdates scheduled weekly Monday 03:00, --auto-stage flag, progress bar 3-step. (5) API: 6 REST endpoints (GET pending, POST approve/reject/schedule/apply, GET validate). (6) Config: GitHub repo settings (GITHUB_REPO_OWNER, GITHUB_REPO_NAME). (7) Documentation: GITHUB_RELEASE_UPDATE_SYSTEM.md (600+ righe comprehensive guide con architettura, workflow, API examples, security, troubleshooting). **Architect-reviewed e production-ready**.
- **October 21, 2025 - Auto-Update System (Legacy)**: Sistema carrier-grade Replit-triggered per gestione automatica deployment e migrations con: (1) Database: system_versions table (12 colonne, 5 indexes) con enum constraints per deployment_status/environment. (2) Model: SystemVersion environment-aware con getCurrentVersion($env), transactional recordDeployment/markAsFailed, automatic rollback su failure. (3) Service: SystemUpdateService con auto-migrations runner, 5 health checks (database/cache/storage/queue/critical_tables), duplicate deployment prevention. (4) CLI: RunSystemUpdate command con progress bar interattiva. (5) API: 6 REST endpoints environment-aware (/status, /version, /health, /history, /update). (6) Integration: build.sh auto-esegue system:update ad ogni deployment. **Sostituito con GitHub Auto-Update System**.
...
- **Software Auto-Update System**: Carrier-grade automatic deployment tracking and migration system. Environment-aware version tracking (local/development/staging/production) with system_versions table. Auto-execution of migrations on deploy via build script integration. Transactional failure handling with automatic rollback to previous stable version. 5-stage health checks (database, cache, storage, queue, critical tables). Duplicate deployment prevention. RESTful API (6 endpoints) and CLI interface (artisan system:update). Real-time deployment history, status monitoring, and rollback capabilities. Architect-reviewed production implementation.
